...
class ReportesDulceria extends StatefulWidget {
  const ReportesDulceria({super.key});

  @override
  State<ReportesDulceria> createState() => _ReportesDulceriaState();
}

class _ReportesDulceriaState extends State<ReportesDulceria> {
  List<Map<String, dynamic>> ventasDulceria = [];
  ...
  Future<void> cargarVentas(String fecha) async {
    ...
    final url = Uri.parse(
      '$baseUrl/api/admin/reporte-dulceria?desde=${desde.toIso8601String()}&hasta=${hasta.toIso8601String()}',
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ventasDulceria = List<Map<String, dynamic>>.from(data);
        });
      } else {
        setState(() => ventasDulceria = []);
      }
    } catch (e) {
  ...
  Future<void> generarPDF() async {
    if (ventasDulceria.isEmpty) return;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Text("Reporte de Ventas Dulcería", style: pw.TextStyle(fontSize: 18)),
          pw.SizedBox(height: 20),
          pw.Table.fromTextArray(
            headers: ['Producto/Combo', 'Cantidad', 'Precio Unitario', 'Total', 'Fecha'],
            data: ventasDulceria.map((e) => [
              e["nombre"] ?? '',
              '${e["cantidad"] ?? 0}',
  ...
  double calcularTotalGeneral() {
    return ventasDulceria.fold(0.0, (acc, e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("📊 Reporte Dulcería", style: TextStyle(color: Colors.white)),
