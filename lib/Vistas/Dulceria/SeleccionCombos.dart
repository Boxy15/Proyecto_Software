...
class ListaVentaDulceriaCombos extends StatefulWidget {
  const ListaVentaDulceriaCombos({super.key});

  @override
  State<ListaVentaDulceriaCombos> createState() => _ListaVentaDulceriaCombosState();
}

class _ListaVentaDulceriaCombosState extends State<ListaVentaDulceriaCombos> {
  ...
  Future<List<Receta>> fetchCombos() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/api/admin/combos?departamento=dulceria'));
  ...
  Widget _buildTarjetaCombo(Receta combo) {
  ...
  Widget buildListaCombos(List<Receta> combos) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) => _buildTarjetaCombo(combos[index]),
    );
  }
        ...
        children: [
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Text('Atrás', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white)),
                        ],
                      ),
                    ...
                    child: FutureBuilder<List<Receta>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return buildListaCombos(snapshot.data!);
                        }
                      },
                    ),
                  ),
          ...
          Positioned(
            ...
            child: ElevatedButton(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const CarritoDulceria())),
